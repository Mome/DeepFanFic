...
class CorpusReader:
    ...
    def count_documents(self):
        num_docs=0
        for folder_name in os.listdir(self.corpus_path):
            story_path = self.corpus_path + '/' + folder_name + '/stories'
            if os.path.isdir(story_path):
                num_docs += len(os.listdir(story_path))
        return num_docs


    def get_iterator(self, yield_meta=True, skip_meta_none=True, **filter_options):
        ...
        for folder_name in os.listdir(self.corpus_path):
            path = self.corpus_path + '/' + folder_name
            ...
            for i, doc_name in enumerate(os.listdir(story_path)):
                ...
                word_list = self.to_machine_readable(text)
    ...
    def to_machine_readable(self, text):
    ...
    def load_encoding(self):
        ...
        path = self.corpus_path + os.sep + 'encoding.npy'
        try:
    ...
    def save_encoding(self, max_dim=0, min_word_freq=0, **filter_options):
        ...
        encoding = self.calculate_encoding(max_dim, min_word_freq, **filter_options)
        path = self.corpus_path + os.sep + 'encoding.npy'
        np.save(path, encoding)
        return encoding


    def calculate_encoding(self, max_dim=0, min_word_freq=0, **filter_options):
        """Generates a 1-of-N encoding from the corpus."""

        corpus_iter = self.get_iterator(yield_meta=False, **filter_options)
    ...
    def get_encode_function(self, encoding):
